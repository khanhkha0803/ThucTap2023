					...
					<form action="index.php?action=updateproduct" method="post" enctype="multipart/form-data">
						<div class="card-body">
							...
							<div class="form-group">
								...
								<select class="custom-select" name="optnsx">
									<option selected>choose</option>
									<?php
										foreach ($listproducer as $producer) :
											extract($producer);
											if ($producer['id'] == $product['idpc'])
												echo ' <option value="' .$producer['id']. '" selected>' .$producer['namepc']. '</option> ';
											else {
												echo ' <option value="' .$producer['id']. '">' .$producer['namepc']. '</option> ';
											}
										endforeach;
									?>
								</select>
							...
							<div class="form-group">
								<label>Price</label>
								<input type="number" class="form-control" name="price" value="<?=$price?>" placeholder="Nhập giá bán">
							</div>
							<div class="form-group">

<?php
function insert_product($namep, $price ,$quantity, $hinh, $info,$idcg,$idpc){
    if($hinh == ""){
        $hinh = "no photo";
    }
    $sql="insert into product(namep,price,quantity,img,info,idcg,idpc) values('$namep', '$price', '$quantity' , '$hinh', '$info', '$idcg','$idpc')";
    pdo_execute($sql);
}
...
function update_product($idp,$namep, $price ,$quantity ,$img, $info ,$idcg,$idpc){
    if($img!="")
        $sql="update product set namep='".$namep."', price='".$price."', quantity='".$quantity."' ,img='".$img."', info='".$info."', idcg='".$idcg."' , idpc='".$idpc."' where idp=".$idp;
    else
        $sql="update product set namep='".$namep."', price='".$price."', quantity='".$quantity."', info='".$info."', idcg='".$idcg."', idpc='".$idpc."' where idp=".$idp;
    pdo_execute($sql);
}
